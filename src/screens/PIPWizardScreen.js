...
export default function PIPWizardScreen({ navigation }) {
  ...
  // Step 1
  const [planType, setPlanType] = useState('Parenting');
  // Step 2
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      navigation.replace('PIPDetail', { plan });
    } catch (err) {
    ...
    <SafeAreaView style={styles.safe} edges={[]}>
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          {step === 1 && (
            <View style={styles.stepWrap}>
              ...
              <Text style={styles.stepTitle}>What area do you want to improve?</Text>
              <View style={styles.optionGrid}>
                {PLAN_TYPES.map(pt => (
                  <TouchableOpacity
                    key={pt.key}
                    style={[styles.optionCard, planType === pt.key && styles.optionCardActive]}
                    onPress={() => setPlanType(pt.key)}
                    activeOpacity={0.75}
                  >
                    <Ionicons name={pt.icon} size={22} color={planType === pt.key ? '#1B3D2F' : '#6B7280'} />
                    <Text style={[styles.optionCardText, planType === pt.key && styles.optionCardTextActive]}>{pt.label}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </View>
        ...
        <View style={[styles.footer, { paddingBottom: insets.bottom + 16 }]}>
          {step < totalSteps ? (
            <TouchableOpacity
              style={[styles.ctaBtn, step === 2 && !userGoal.trim() && styles.ctaBtnDisabled]}
              onPress={() => {
                if (step === 2 && !userGoal.trim()) { setError('Please describe your goal or struggle.'); return; }
                setError('');
                setStep(s => s + 1);
              }}
              activeOpacity={0.85}
